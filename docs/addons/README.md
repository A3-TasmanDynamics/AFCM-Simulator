...
|---|---|---|---|
| `main` | `afcm_sim_main` | `cba_main` | Owns the backend interface (`afcm_sim_fnc_backend_registerBackend`/`selectBackend`/`applyInjury`/`removeInjury`/`getState`/`reset`/`setUnconscious`/`getActive`) that every other addon calls through, plus the CBA Addon Options settings (`afcm_sim_debugLogging`, `afcm_sim_defaultInjuryLevel`, `afcm_sim_defaultCasualtyType`). Also sets the real ACE setting `ace_medical_spontaneousWakeUpChance` to 0 mission-wide (`fnc_disableSpontaneousWakeup.sqf` — see REFERENCES.md for why). The only addon every other AFCM-Simulator addon ultimately depends on |
| `ui` | `afcm_sim_ui` | `cba_main` | Dialog framework + event bus (`afcm_sim_ui_fnc_publish`/`subscribe`). Four real dialogs, all true-centered on screen: limb-select ("main" screen — button-per-limb, not a silhouette, plus Presets and full-patient Reset Patient buttons), the injury editor (wound type/severity/bleeding, always shown; Back returns to limb-select instead of closing outright; Save as Preset; a purely-local, non-destructive Reset Limb; plus KAT-only Fracture/Pneumothorax controls shown only when KAT is active — Apply disabled with an explanation if no usable backend is active at all), the Preset Library (apply/delete/export/import — also has an MCI batch mode, applying the picked preset to a whole `AFCM_SIM_UI_targetUnits` array in one Apply instead of a single patient), and a small Preset Save name-prompt. Entry points are vanilla `addAction`s on spawned patients ("Edit Injuries" per patient, "Assign MCI Preset" on a whole freshly-spawned batch — `zeus`'s MCI Spawner module), not the ACE interaction menu |
| `scenario` | `afcm_sim_scenario` | `cba_main`, `afcm_sim_main` | Domain logic — `afcm_sim_scenario_fnc_randomizeInjuries` rolls a real `Injury` array from an injury-level profile ([DESIGN.md §4.4](../DESIGN.md#44-injury-levels-randomization-difficulty)); `serverApplyInjury`/`serverReset` are the server-authoritative request handlers the injury editor uses; `serverApplyKatFracture`/`serverApplyKatPneumothorax` are the same for KAT-only state (INJURY_CODES.md §6), guarding on KAT actually being the active backend before calling directly into `kat_compat`. Also owns the Injury Preset library (INJURY_CODES.md §4) — `getBuiltinPresets`/`getUserPresets`/`saveUserPreset`/`deleteUserPreset`/`findPreset`/`exportPreset`/`importPreset`, plus `serverApplyPreset` (loops `serverApplyInjury` once per injury in the preset) |

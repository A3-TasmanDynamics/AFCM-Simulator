...
| `spawner` | `afcm_sim_spawner` | `cba_main`, `afcm_sim_main`, `afcm_sim_scenario` | Patient spawning/clearing — `spawnPatient`, `spawnRandomPatient`, `clearAllPatients`. Owns `AFCM_SIM_patientGroup`/`AFCM_SIM_spawnedPatients`. Server-authoritative. `spawnPatient` also runs a recurring 3s safeguard per patient that re-locks `setUnconscious true` for as long as it exists (REFERENCES.md — belt-and-suspenders against "healing itself" after two targeted upstream fixes didn't fully hold up) |

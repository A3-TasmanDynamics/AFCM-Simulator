...
| `ui` | `afcm_sim_ui` | `cba_main` | Dialog framework + event bus (`afcm_sim_ui_fnc_publish`/`subscribe`). Four real dialogs, all true-centered on screen: limb-select ("main" screen — button-per-limb, not a silhouette, plus Presets and full-patient Reset Patient buttons), the injury editor (wound type/severity/bleeding, always shown; Save as Preset; a purely-local, non-destructive Reset Limb; plus KAT-only Fracture/Pneumothorax controls shown only when KAT is active — Apply disabled with an explanation if no usable backend is active at all), the Preset Library (apply/delete/export/import), and a small Preset Save name-prompt. Entry point is a vanilla `addAction` on spawned patients, not the ACE interaction menu |
...
| Addon | PBO | `requiredAddons` | What it does |
|---|---|---|---|
| `zeus` | `afcm_sim_zeus` | `cba_main`, `afcm_sim_main`, `afcm_sim_scenario`, `afcm_sim_spawner` | "Spawn Patient" Zeus module (class still named `AFCM_SIM_ModuleSpawnRandomPatient` internally) — spawns clean/unconscious, injuries picked afterward via the "Edit Injuries" action, not randomized. Has a "Casualty Type" attribute (Civilian/Military BLUFOR/OPFOR/Independent, `AFCM_SIM_CasualtyTypeAttributes`) for clothing/appearance only. Category grouping via `CfgFactionClasses` + `side = 7`, not `CfgVehicleClasses` (Zeus-specific gotcha, not an Eden mechanism — see the module's own `config.cpp` comments) |
| `eden` | `afcm_sim_eden` | `cba_main`, `afcm_sim_main`, `afcm_sim_scenario`, `afcm_sim_spawner` | "AFCM Patient" (spawns clean, same "Edit Injuries" flow as Zeus's Spawn Patient — no injury-level attribute anymore, but keeps a "Casualty Type" one) and "AFCM MASCAL Zone" (patient-count + injury-level + casualty-type attributes, still randomized — batch/mass-casualty drills are the one place that still makes sense) Eden modules. Both also set `scopeCurator = 2` so they're Zeus-placeable too |

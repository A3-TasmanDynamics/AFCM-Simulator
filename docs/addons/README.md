...
| `ui` | `afcm_sim_ui` | `cba_main` | Dialog framework + event bus (`afcm_sim_ui_fnc_publish`/`subscribe`). Two real dialogs: limb-select (button-per-limb, not a silhouette) and the injury editor (wound type/severity/bleeding — deliberately ACE-only selections, works identically for ACE/KAT; Apply disabled with an explanation if no usable backend is active) it opens next. Entry point is a vanilla `addAction` on spawned patients, not the ACE interaction menu |
...
| **[`kat_compat`](KAT_COMPAT.md)** | `afcm_sim_kat_compat` | `ace_medical_engine`, `kat_main` | 15 | **`applyInjury`/`getState` are real** — identical to `ace_compat`'s, since KAT extends ACE's own wound pipeline rather than replacing it. `removeInjury` is a stub. Real KAT-only fracture/pneumothorax functions were built and reverted (KAT_COMPAT.md §5) in favour of an ACE-only UI. **Full doc: [KAT_COMPAT.md](KAT_COMPAT.md)** |

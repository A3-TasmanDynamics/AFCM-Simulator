 ...
 * replacing it, so ace_medical_fnc_fullHeal applies correctly here too) - PLUS clearing the real
 * KAT-only state (INJURY_CODES.md §6) that has no ACE equivalent, so ace_medical_fnc_fullHeal
 * cannot and does not clear it.
 *
 * Real, confirmed bug fixed here: this used to only call fullHeal/setUnconscious, leaving
 * kat_surgery_fractures/kat_breathing_pneumothorax(+hemo/tension)/kat_airway_obstruction(+occluded)
 * completely untouched - a patient given a Comminuted fracture, then Reset, kept showing

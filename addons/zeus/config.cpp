...
class CfgVehicles
{
    ...
    class AFCM_SIM_CasualtyTypeAttributes: Module_F
    {
        ...
        // AFCM_SIM_ModuleSpawnRandomPatient/ModuleMciSpawner/ModuleEditInjuries below), which is
        // what actually supplies a real `model` entry from the engine's own base config chain.
        // Without any inheritance, this class had no `model` anywhere in its lineage, which
        // surfaced as a real, visible "No entry
        // 'bin\config.bin/CfgVehicles/AFCM_SIM_CasualtyTypeAttributes.model'" error dialog on the
        // main menu (confirmed from a client screenshot) - `scope=0` only hides a class from the
        // Zeus/Eden object browser, it does NOT exempt a class from the engine's own property
        // validation, contrary to what this comment used to claim. `: Module_F` fixes that at the
        // source; `scope = 0` is kept for its real, original purpose - this is still never meant to

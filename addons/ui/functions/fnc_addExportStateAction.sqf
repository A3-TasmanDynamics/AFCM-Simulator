 ...
 *
 * The action itself runs entirely locally, no remoteExec needed - fnc_exportPatientState.sqf just
 * reads back AFCM_SIM_appliedInjuries, a publicVariable'd unit variable already replicated to every
 * client (fnc_backend_applyInjury.sqf). Copies the result straight to the OS clipboard via the real
 * `copyToClipboard` command, ready to paste into an Eden AFCM Patient module's Injury Preset Import
 * attribute or the Preset Library's own Import field.
 *
 * Arguments:
...
_unit addAction [
    "<t color='#c1272d'>AFCM: Export Patient State</t>",
    {
        ...
        if (_exported isEqualTo "") exitWith {
            ["Export Patient State", "No AFCM-applied injuries on this patient yet."] call afcm_sim_ui_fnc_showToast;
        };

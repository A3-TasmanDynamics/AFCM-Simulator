 ...
 * KAT-specific - shown only when "chest" is among the selected limbs, same gating as Pneumothorax
 * (the real ace_medical_vitals_inCardiacArrest flag is actually whole-patient, but chest is where
 * an instructor thinks to look for it); its options differ by backend (ACE: None/Cardiac Arrest;
...
[{
    ...
    // Shared (ACE + KAT), chest-gated - same "chest" limb-gating as Pneumothorax, since the
    // instructor thinks of cardiac arrest as a chest/torso condition even though the real
    // ace_medical_vitals_inCardiacArrest flag is actually whole-patient. Options depend on backend:
    // ACE has no rhythm concept (afcm_sim_ace_fnc_applyCardiacState only takes a bool), so it only
    // offers None/Cardiac Arrest; KAT offers the full real 0-4 rhythm enum
    ...
    // fnc_applyAirway.sqf). Cardiac State: chest only, ACE or KAT alike - see the comment above.
    // Hidden rows just leave blank space in the panel (Arma dialogs are absolute-positioned, not
    // flow-layout) rather than reflowing it.
    ...
    private _showCardiac = (_backend in ["ace", "kat"]) && {"chest" in _limbs};

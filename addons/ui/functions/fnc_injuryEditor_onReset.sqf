 ...
 * Purely local - clears the wound type/severity/bleeding fields, plus Cardiac State and Fracture/
 * Pneumothorax/Airway if visible (KAT only), back to their defaults (same defaults
 * fnc_injuryEditor_init.sqf sets on open: Gunshot / Moderate / unchecked / None / None / None /
 * None) so an instructor can reconsider what to apply to this limb, without touching the patient's
 * actual medical state at all.
 *
...
(_display displayCtrl 25) lbSetCurSel 0;
